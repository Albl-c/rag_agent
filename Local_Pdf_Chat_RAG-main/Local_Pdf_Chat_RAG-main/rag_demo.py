...
from core.embeddings import encode_texts, EMBED_MODEL_NAME
...
def init_vector_store():
    """启动时尝试加载持久化索引"""
    loaded, message = vector_store.load_index(INDEX_STORAGE_PATH)
    if loaded:
        bm25_loaded = bm25_manager.load(INDEX_STORAGE_PATH)
        logging.info("启动加载索引成功: %s", message)
        if bm25_loaded:
            logging.info("启动加载 BM25 索引成功")
    else:
        logging.info("启动未加载索引: %s", message)
    return loaded, message


_INITIAL_LOAD_OK, _INITIAL_LOAD_MSG = init_vector_store()


# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 文档处理
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def process_multiple_files(files, index_mode="append", progress=gr.Progress()):
    """处理多个文件：提取文本 → 分块 → 向量化 → 构建索引"""
    if not files:
        return "请选择要上传的文件(支持PDF, Word, Excel, PPT, TXT, Markdown等)", []

    try:
        if index_mode not in {"append", "rebuild"}:
            index_mode = "append"

        if index_mode == "rebuild":
            progress(0.1, desc="重建模式：清理历史数据...")
            vector_store.clear()
            bm25_manager.clear()
        else:
            progress(0.1, desc="追加模式：保留历史数据...")
        ...
        for idx, file in enumerate(files, 1):
            try:
                ...
                selected_strategy, route_features = route_chunk_strategy(file_name, text)
                chunks = split_text(text, strategy=selected_strategy)
                doc_id = f"doc_{time.time_ns()}_{idx}"
                metadatas = [{
                    "source": file_name,
                    "doc_id": doc_id,
                ...
                processed_results.append(
                    f"✅ {file_name}: 成功处理 {len(chunks)} 个文本块 "
                    f"(策略={selected_strategy}, 标题={route_features['markdown_header_count']}, "
                    f"结构标签={route_features['structured_label_count']})"
                )
        ...
        if all_chunks:
            ...
            if index_mode == "rebuild" or not vector_store.is_ready:
                vector_store.build_index(all_chunks, all_ids, all_metadatas, embeddings)
            else:
                vector_store.append_index(all_chunks, all_ids, all_metadatas, embeddings)

        # progress(0.95, desc="构建BM25检索索引...")
        # bm25_manager.build_index(all_chunks, all_ids)
        vector_store.save_index(INDEX_STORAGE_PATH)
        bm25_manager.save(INDEX_STORAGE_PATH)

        summary = f"\n总计处理 {total_files} 个文件，{len(all_chunks)} 个文本块（模式: {index_mode}）"
...
def get_system_models_info():
    ...
    return {
        ...
        "分块方法": "自动路由: markdown_header / structured_recursive / recursive（默认回退）",
...
.upload-file-box { height: 190px !important; overflow-y: auto !important; }
.upload-file-box > div { height: 100% !important; }
.upload-file-box [data-testid="file-upload"], .upload-file-box [data-testid="files"] {
    max-height: 150px !important;
    overflow-y: auto !important;
}
.fixed-scroll-box textarea {
...
with gr.Blocks(title="科研资料问答助手", css=CSS, js=THEME_JS) as demo:
    with gr.Row():
        with gr.Column(scale=9):
            gr.Markdown("# 🧠 科研资料问答助手")
                ...
                with gr.Column(scale=5, elem_classes="left-panel"):
                    ...
                    with gr.Group():
                        file_input = gr.File(
                            ...
                            file_count="multiple",
                            elem_classes="upload-file-box"
                        )
                        index_mode_dropdown = gr.Dropdown(
                            choices=[("追加模式（保留旧知识库）", "append"), ("重建模式（清空后重建）", "rebuild")],
                            value="append",
                            label="索引写入模式"
                        )
                        ...
                        file_list = gr.Textbox(
                            ...
                            lines=6,
                            max_lines=6,
                            elem_classes=["file-list", "fixed-scroll-box"]
                        )
    ...
    upload_btn.click(
        ...
        inputs=[file_input, index_mode_dropdown],
